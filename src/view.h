...
// LOCALPOLYGON
// a local class for constructing an on the fly Polygon to use for the horizon.
...
private:
	...
	int intersects(int x1_1, int y1_1, int x2_1, int y2_1, int x1_2, int y1_2, int x2_2, int y2_2);
	...
	void DrawHorizon(int xangle, int yangle, int zangle, unsigned char *screen)
	{
	...
	void DrawPolygonList(unsigned char *screen)
	{
	...
	void ZClip(Polygon *polygon, ClippedPolygon *clip, int zmin = 1)
	{
		...
		// Transfer information from polygon's polybase class to clipped polyBase class:
		*clip = *polygon;
...
public:
	...
	{
		// Clear the viewport:
		clrwin(xmin, ymin, xmax - xmin, ymax - ymin, screen_buffer);

		// If horizon desired, draw it:
		if (horizon_flag) {
			DrawHorizon(curview.xangle, curview.yangle, curview.zangle, screen_buffer);
		}
	...
	// allow external reset of palette to world
	void PaletteReset()
	{
		world.SetWorldPalette();
	}
